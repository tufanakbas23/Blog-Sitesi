...
class Homepage extends Controller
{
  ...
  public function contactpost(Request $request)
  {
    ...
    $body = 'Mesajı Gönderen: ' . $request->name . '<br/>' .
      'E-posta: ' . $request->email . '<br/>' .
      'Konu: ' . $request->topic . '<br/>' .
      'Mesaj: ' . $request->message . '<br/><br/>' .
      'Gönderilme Tarihi: ' . now();

    try {
      Mail::raw(strip_tags($body), function ($message) use ($request) {
        $message->to('mehmettufanakbas248@gmail.com');
